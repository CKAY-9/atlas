...
use atlas_db_crud::{users::get_user_with_token, classrooms::get_classroom_with_id};
use atlas_utils::extract_header_value;
use reqwest::StatusCode;
use crate::dto::Message;
use super::dto::{GetClassroomDTO, GetClassroomMessage};

#[get("")]
pub async fn get_classroom_from_id(request: HttpRequest, query: web::Query<GetClassroomDTO>) -> Result<impl Responder, Box<dyn std::error::Error>> {
    ...
    let user_option = get_user_with_token(connection, token.unwrap());
    match user_option {
        Some(user) => {
            let classroom_option = get_classroom_with_id(connection, query.classroom_id);
            ...
            Ok(HttpResponse::Ok().json(GetClassroomMessage { message: "Fetched classroom".to_string(), classroom }))
        },
        None => {
