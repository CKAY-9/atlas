...
use atlas_db_crud::{users::get_user_with_token, classrooms::get_classroom_with_id, units::create_new_unit};
use atlas_db_schema::models::{User, Classroom, NewCourseUnit, CourseUnit};
...
#[post("")]
pub async fn new_course_unit(request: HttpRequest, data: web::Json<NewUnitDTO>) -> Result<impl Responder, Box<dyn std::error::Error>> {
        ...
        Some(user) => { 
            ...
            match classroom_option {
                Some(classroom) => {
                    ...
                    match insert_option {
                        Some(insert) => Ok(HttpResponse::Ok().json(NewUnitMessage { message: "Created unit".to_string(), unit_id: insert.id })),
